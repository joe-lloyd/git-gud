...
export const GraphView: React.FC<GraphViewProps> = ({
  ...
  const numLanes = useMemo(() => nodes.reduce((m, n) => Math.max(m, n.lane), 0) + 1, [nodes])
  ...
  const endRow = Math.min(nodes.length - 1, Math.ceil((scrollTop + viewHeight) / ROW_H) + OVERSCAN)
  ...
  return (
    <div className="graph-view" ref={containerRef}>
      {/* Column headers */}
      <div className="graph-header" style={{ paddingLeft: graphWidth + 8 }}>
      ...
      <div className="graph-scroll" ref={scrollRef} onScroll={handleScroll}>
        ...
        {/* Canvas — sticky left, draws graph lines/nodes */}
        <div className="graph-canvas-wrap" style={{ width: graphWidth }}>
          <canvas ref={canvasRef} className="graph-canvas" />
        </div>

        {/* Virtualized DOM rows */}
        <div className="graph-rows" style={{ marginLeft: graphWidth }}>
          {/* Top spacer */}
...
const CommitRow: React.FC<CommitRowProps> = React.memo(({ node, isSelected, onSelect, onContextMenu, worktreeBranches }) => {
  const { commit } = node
  const groups = useMemo(() => groupRefs(commit.refs, worktreeBranches), [commit.refs, worktreeBranches])
  ...
  return (
    <div
      ...
      onContextMenu={onContextMenu ? (e) => onContextMenu(e, commit.sha) : undefined}
    >
      {/* Ref pills — grouped */}
...
function groupRefs(refs: string[], worktreeBranches: Set<string>): RefGroup[] {
  ...
  for (const ref of refs) {
    if (ref === 'HEAD') { headTarget = 'HEAD_standalone'; continue }
  ...
  for (const ref of refs) {
    ...
    if (ref.startsWith('tag: ')) {
      ...
      groups.set(ref, {
        key: ref, name: tagName,
        isHead: false, hasLocal: false, hasRemote: false,
        isTag: true, hasWorktree: false,
        tooltip: ref,
      })
      continue
    }
  ...
  if (headTarget === 'HEAD_standalone') {
    groups.set('HEAD', {
      key: 'HEAD', name: 'HEAD',
      isHead: true, hasLocal: false, hasRemote: false,
      isTag: false, hasWorktree: false,
...
function RefPill({ group }: { group: RefGroup }) {
  if (group.isTag) {
    return (
      <span className="ref-pill ref-tag" title={group.tooltip}>
  ...
  return (
    <span className={cls} title={group.tooltip}>
      {group.isHead    && <span className="rp-icon rp-head">◉</span>}
      {group.hasLocal  && <span className="rp-icon rp-local">⎇</span>}
      {group.hasRemote && <span className="rp-icon rp-remote">↑</span>}
      {group.hasWorktree && <span className="rp-icon rp-worktree" title="Checked out in worktree">⊞</span>}
