...
- [x] 5.9 Full remote control: every public GitService write runs on the host (explicit `WRITE_METHODS`, coverage-guarded by test; `commitStreaming` callback substituted host-side); host watches the working tree for peer subscribers so edits show up on the peer in ~1 s. Verified live: host edit → client 1.3 s; stage/unstage/discard/commit from client executed on host.
- [x] 5.10 Internet reach (GUI↔GUI): names never clobbered by LAN beacons (`isIpLiteral`), in-session LAN fallback, Tailscale/SSH recipe in Connect-by-address, spec + design §9
