...
`openspec/specs/` is empty. Several in-flight changes (`git-feature-coverage`, `clone-audit-and-complete`, `add-commit-hook-output`, `add-syntax-highlighting`) describe intended work but none have been archived to canonical specs, and a burst of recent UI work (resizable panels, theming/settings, graph rendering, multi-select, worktree management) shipped without written requirements. This change writes down the behavior the code already exhibits so future changes have a regression baseline.
...
- **Five capabilities, named to avoid collision** with the as-yet-unarchived capabilities in other changes (e.g. this baseline uses `commit-graph` for the concrete render/interaction behavior; graph *performance* internals stay with `clone-audit-and-complete`). On archive, overlaps will be reconciled deliberately.
...
- At archive time, should this baseline's `commit-graph` be merged with `clone-audit-and-complete`'s graph capabilities, or kept as the concrete-behavior layer beneath the performance spec? (Defer to the bug/test change.)
